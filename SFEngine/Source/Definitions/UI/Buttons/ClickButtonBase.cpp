...
namespace Engine
{
  ...
  namespace UI
  {
    ...
    std::shared_ptr<ClickButtonBase> ClickButtonBase::Create(std::shared_ptr<WidgetHelper> ThisHelper, const sf::Vector2f &Position, const sf::Vector2f &Size)
    {
      DEBUG_ONLY std::cerr << "ClickButtonBase::Create" << std::endl;

      if (!ThisHelper || !ThisHelper->CanAcceptWidget())
      ...
      Widget->Creator = ThisHelper;
      ThisHelper->RegisterWidget(Widget);

      DEBUG_ONLY std::cerr << "ClickButtonBase::Create : Adding ColoredQuad : {" << Position.x << ", " << Position.y << "}, {" << Size.x << ", " << Size.y << "}" << std::endl;
