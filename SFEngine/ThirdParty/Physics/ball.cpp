...
void ball::draw( sf::RenderWindow& rRW )const { rRW.draw(img); }
...
// check for collision. Check distance between centers.
    ...
    if( ballFree.v.x < 0.0f ) ballFree.v.x *= -Cr;
...
//    vec2d L = B - A;
    ...
    if( ( d < 0.0f ) || ( d > magL ) ) return false;// center between ends only
...
bool ball::is_inMe( const arcSeg& AS, vec2d& Pimp, vec2d& Nh, float& dSep )const// code goes here. Impact point is written.
{
 ...
 //   float R = ( AS.s[0] ).mag();
    ...
    if( sep.cross(AS.s[0]) < 0.0f || sep.cross(AS.s[1]) > 0.0f ) return false;// ends missed
